...
export class AttributePanelComponent implements OnInit {
  ...
  ngOnInit(): void {
    if(this._attribute.id == 0 || this._attribute.id == undefined){
      ...
      if(this._attribute.attributeType.id == 1){
        ...
        for (elemento of this._attribute.attributeOptions) {
          this._attributeoptionSelected.push(elemento.id);
        }
  ...
  saveAttribute(): void{
    this.submitted = true;
    if(this._attribute.name.trim() && this._attribute.abbreviation.trim() && ((this._attributeoptionSelected.length ==0 && this._attribute.attributeType.id != 1) || (this._attributeoptionSelected.length > 0 && this._attribute.attributeType.id == 1))){
      ...
      this._attributeService.postAttribute(this._attribute).subscribe((data: number) => {
        if(data > 0) {
          ...
          this._attribute = new Attribute();
          this._attribute.active = true;
          this._attributeService.getAttributebyfilter(this.refreshPOT = new AttributeFilter(), OrderCodes.UpdatedDate).subscribe((data: Attribute[]) => {
            this._attributeService._AttributeList = data;
          });
          this.submitted = false;
        }else if(data == -1){
          this.messageService.add({severity:'error', summary:'Error', detail: "Este atributo ya existe"});
        }else{
