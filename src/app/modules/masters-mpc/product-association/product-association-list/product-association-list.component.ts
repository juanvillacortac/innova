...
export class ProductAssociationListComponent implements OnInit {
  ...
  constructor(private router: Router, 
    ...
    this.breadcrumbService.setItems([
      { label: 'OSM' },
      { label: 'MPC' },
      { label: 'Asociación de productos', routerLink: ['/product-association-list'] }
    ]);
  }
  ...
  search(){
    this.loading = true;
    this.loading = false;
    this._productassociationservice.getProductassociationbyfilter(this._productassociationFilters, OrderCodes.UpdatedDate).subscribe((data: Productassociation[]) => {
