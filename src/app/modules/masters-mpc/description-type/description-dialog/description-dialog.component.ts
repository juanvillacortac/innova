...
export class DescriptionDialogComponent implements OnInit {
  ...
  constructor(private _descriptionservice: DescriptionTypeService, private messageService: MessageService) { }
          ...
          this._descriptionservice.postDescription(this._description).subscribe((data: number) => {
            if (data > 0) {
              ...
              this._descriptionservice.getDescriptionbyfilter(this.filters).subscribe((data: DescriptionType[]) => {
                this._descriptionservice._descriptionList = data;

              });
              this.submitted = false;
            }else if (data == -1){
              this.messageService.add({severity:'error', summary:'Alerta', detail: "El tipo de descripción ya existe."});
