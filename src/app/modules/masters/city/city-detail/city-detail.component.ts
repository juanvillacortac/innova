  ...
  onEditForm(){  
    ...
    this.getStatesListPromise(filters).then(()=>{
        this.getDistrictListPromise(new DistrictFilters).then(()=>{
            this.countrySelected= true;    
            this.stateSelected= true;    
            let district = this.district.value.find(p=> Number(p.id) === Number(this.city.district.id));     
            this.cityForm.patchValue({
                    id: this.city.id,
                    name: this.city.name,
...
getCountriesPromise = (filters: CountryFilter) => {
    ...
    if(this.isValidateCity(newCity))
    {
      this._cityService.addCity(newCity).subscribe((data: number) => {
        if(data > 0) {
          ...
          this.messageService.add({key:'city', severity:'warn', summary:'Alerta', detail: "Esta ciudad ya existe"});
        }else{
  ...
  {
      let inValidateName = this.cityList.find(p=> p.name.trim().toUpperCase() === newCity.name.trim().toUpperCase() && p.district.id === newCity.district.id && p.id !== newCity.id);   
      let inValidateAbbreviation = this.cityList.find(p=> p.abbreviation.trim().toUpperCase() === newCity.abbreviation.trim().toUpperCase() && p.id !== newCity.id);
      
      if(inValidateName)
      {
        this.messageService.add({key:'city', severity:'warn', summary:'Alerta', detail: "Ya existe una ciudad con el nombre ingresado"});
        return false;
      }

      if(inValidateAbbreviation)
      {
        this.messageService.add({key:'city', severity:'warn', summary:'Alerta', detail: "Ya existe una ciudad con esa abreviatura ingresada"});
        return false;
      }
