...
getCountriesPromise = (filters: CountryFilter) => {
  ...
  isValidateDistrict(newDistrict: District)
  {
      ...
      if(inValidateName)
      {
        this.messageService.add({key:'district', severity:'warn', summary:'Alerta', detail: "Ya existe un municipio con el nombre ingresado"});
        return false;
      }

      if(inValidateAbbreviation)
      {
        this.messageService.add({key:'district', severity:'warn', summary:'Alerta', detail: "Ya existe un municipio con esa abreviatura ingresada"});
        return false;
      }
