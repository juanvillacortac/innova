...
getCountriesPromise = () => {
  ...
  getPriceGroupingListPromise = () => {
    ...
    .then(results => {
      this.pricegrouping.value = results;
    }, (error) => {
    ...
    if(this.isValidatePricetype(newPricetype))
    {
      this._priceTypeService.addPriceType(newPricetype).subscribe((data: number) => {
        if(data > 0) {
          ...
          this.messageService.add({key:'price-type', severity:'warn', summary:'Alerta', detail: "Este tipo de precio ya existe"});
        }else{
  ...
  isValidatePricetype(newPricetype: PriceType)
  {
      ...
      if(inValidateName)
      {
        this.messageService.add({key:'price-type', severity:'warn', summary:'Alerta', detail: "Ya existe un tipo de precio con el nombre ingresado"});
        return false;
      }

      if(inValidateAbbreviation)
      {
        this.messageService.add({key:'price-type', severity:'warn', summary:'Alerta', detail: "Ya existe un tipo de precio con esa abreviatura ingresada"});
        return false;
      }
  ...
  private setNewPriceTypeForm() {
      return this.formBuilder.group({
        ...
        selectedPricegrouping:['', Validators.required],
