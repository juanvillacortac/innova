...
getCountriesPromise = () => {
  ...
  getEntityTypeListPromise = () => {
    ...
    .then(results => {
      this.entityTypes.value = results;
    }, (error) => {
    ...
    if(this.isValidateDocumentType(newDocumentType))
    {
      this._documentTypeService.addDocumentType(newDocumentType).subscribe((data: number) => {
        if(data > 0) {
          ...
          this.messageService.add({key:'document-type', severity:'warn', summary:'Alerta', detail: "Este tipo de documento ya existe"});
        }else{
  ...
  {
      let inValidateName = this.documentTypeList.find(p=> p.name.trim().toUpperCase() === newDocumentType.name.trim().toUpperCase() && p.id !== newDocumentType.id);   
      let inValidateAbbreviation = this.documentTypeList.find(p=> p.identifier.trim().toUpperCase() === newDocumentType.identifier.trim().toUpperCase() && p.id !== newDocumentType.id);
      
      if(inValidateName)
      {
        this.messageService.add({key:'document-type', severity:'warn', summary:'Alerta', detail: "Ya existe un tipo de documento con el nombre ingresado"});
        return false;
      }

      if(inValidateAbbreviation)
      {
        this.messageService.add({key:'document-type', severity:'warn', summary:'Alerta', detail: "Ya existe un tipo de documento con esa abreviatura ingresada"});
        return false;
      }
  ...
  private setNewDocumentTypeForm() {
      return this.formBuilder.group({
        ...
        selectedEntityType:['', Validators.required],
